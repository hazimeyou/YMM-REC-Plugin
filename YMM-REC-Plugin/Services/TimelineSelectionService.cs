    ...
    public class TimelineSelectionService
    {
        ...
        public bool TryMoveToNextSerif(string? currentSerif, out string? nextSerif)
        {
            ...
            try
            {
                ...
                if (timeline is null)
                {
                    LogService.Debug("TimelineSelection: TryMoveToNextSerif skipped. TimelineInstance is null");
                    return false;
                }
                ...
                if (selected is null)
                {
                    LogService.Debug("TimelineSelection: TryMoveToNextSerif skipped. no selected voice item");
                    return false;
                }
                ...
                {
                    LogService.Debug("TimelineSelection: TryMoveToNextSerif skipped. no voice items in timeline");
                    return false;
                }
                ...
                if (string.IsNullOrWhiteSpace(serif))
                {
                    LogService.Debug("TimelineSelection: TryMoveToNextSerif next item has empty serif");
                    return false;
                }
                ...
                try
                {
                    ...
                    if (!selectionMoved)
                        LogService.Debug("TimelineSelection: next serif prepared without selection move");
                }
                catch (Exception ex)
                {
                    LogService.Debug($"TimelineSelection: selection move skipped. {ex.GetType().Name}: {ex.Message}");
                }

                var frameMoved = TryMoveCurrentFrame(next);
                LogService.Write($"TimelineSelection: moved to next serif. length={serif.Length}, selectionMoved={selectionMoved}, frameMoved={frameMoved}, currentFrame={currentFrame}");
                return true;
            }
            catch (Exception ex)
            {
                LogService.Write("TimelineSelection: TryMoveToNextSerif failed", ex);
                return false;
            }
        }
        ...
        public IReadOnlyList<object> GetVoiceItemsSnapshot()
        {
            ...
            {
                LogService.Write("TimelineSelection: GetVoiceItemsSnapshot failed", ex);
                return Array.Empty<object>();
            }
        }
        ...
        private static bool TrySetSelection(object item)
        {
            ...
            LogService.Debug("TimelineSelection: TrySetSelection failed on all known roots");
            return false;
        }
                ...
                try
                {
                    if (name == "SelectedItem" && property.CanWrite)
                    {
                        if (property.PropertyType.IsInstanceOfType(item) || property.PropertyType == typeof(object))
                        {
                            ...
                            LogService.Debug($"TimelineSelection: set {name} on {source.GetType().FullName}");
                            return true;
                        }
                    }
                    ...
                    {
                        LogService.Debug($"TimelineSelection: replaced {name} collection on {source.GetType().FullName}");
                        return true;
                    }
                }
                catch
