...
namespace YMM_REC_Plugin.Services
{
    public class VoiceTimelineInsertService
    {
        ...
        private static void SetBoolMember(object target, string name, bool value)
        {
            ...
            LogService.Debug($"VoiceTimelineInsert: bool member not found. target={target.GetType().FullName}, name={name}");
        }
        ...
        private static void UpdateSelectedVoiceItemLength(object selected, RecordingScriptItem item)
        {
            try
            {
                ...
                LogService.Write(
                    $"VoiceTimelineInsert: updated selected item length. frames={frames}, voiceDuration={duration}, contentDuration={contentDuration}, tailPadding={tailPadding}, fps={fps:0.###}, " +
                    $"lengthUpdated={updatedLength}, voiceLengthUpdated={updatedVoiceLength}, contentLengthUpdated={updatedContentLength}, originalUpdated={updatedOriginalContentLength}");
            }
            catch (Exception ex)
            {
                LogService.Write("VoiceTimelineInsert: update length failed", ex);
            }
        }
        ...
        private static TimeSpan ResolveTailPadding(object selected, double fps)
        {
            ...
            try
            {
                ...
                if (fromCharacter.HasValue && fromCharacter.Value > TimeSpan.Zero)
                {
                    ...
                    LogService.Write($"VoiceTimelineInsert: tailPadding from character setting={fromCharacter.Value} (source={characterSource}), applied={clamped}");
                    return clamped;
                }
        ...
        private static void TryRequestVoiceGeneration(object selected)
        {
            try
            {
                ...
                LogService.Write("VoiceTimelineInsert: no direct voice-generation trigger found on selected item");
            }
            catch (Exception ex)
            {
                LogService.Write("VoiceTimelineInsert: TryRequestVoiceGeneration failed", ex);
            }
        }
                    ...
                    _ = task.ContinueWith(
                        t =>
                        {
                            if (t.IsFaulted)
                            {
                                ...
                                LogService.Write($"VoiceTimelineInsert: trigger method faulted. method={name}", ex);
                            }
                        },
                        TaskScheduler.Default);
                }

                LogService.Write($"VoiceTimelineInsert: voice-generation requested via method {name}");
                return true;
            }
        ...
        private static bool TryExecuteVoiceGenerationCommand(object target)
        {
            ...
            foreach (var name in candidateNames)
            {
                ...
                LogService.Write($"VoiceTimelineInsert: voice-generation requested via command {name}");
                return true;
            }
